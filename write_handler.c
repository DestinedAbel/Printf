	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS && padd == ' ')/* Asign extra char to left of buffer */
		{
			...
			return (write(1, &buffer[index], length) + write(1, &buffer[1], l - 1));
		}
		else if (!(flags & F_MINUS) && padd == ' ')/* extra char to left of buff */
		{
			if (extra_c)
				buffer[--index] = extra_c;
			return (write(1, &buffer[1], l - 1) + write(1, &buffer[index], length));
		}
...
int write_unsgnd(int is_negative, int index,
	...
	int flags, int width, int precision, int size)
{
	/* The number is stored at the bufer's right and starts at position l */
	int length = BUFF_SIZE - index - 1, l = 0;
	char padd = ' ';
	...
	if (width > length)
	{
		...
		{
			return (write(1, &buffer[index], length) + write(1, &buffer[0], l));
		}
		else /* Asign extra char to left of padding [padd>buffer]*/
		{
			return (write(1, &buffer[0], l) + write(1, &buffer[index], length));
		}
	}
	...
	int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS && padd == ' ')/* Asign extra char to left of buffer */
		{
			...
			if (extra_c)
				buffer[--index] = extra_c;
			return (write(1, &buffer[index], length) + write(1, &buffer[3], l - 3));
		}
		else if (!(flags & F_MINUS) && padd == ' ')/* extra char to left of buffer */
		{
			...
			if (extra_c)
				buffer[--index] = extra_c;
			return (write(1, &buffer[3], l - 3) + write(1, &buffer[index], length));
		}
		else if (!(flags & F_MINUS) && padd == '0')/* extra char to left of padd */
		{
			if (extra_c)
